...
async function checkServerHealth(): Promise<void> {
    try {
        ...
        if (data.status === 'ok' && data.geminiConfigured) {
            ...
            (statusIndicator.querySelector('.status-dot') as HTMLElement).style.background = 'var(--success)';
        } else if (data.status === 'ok' && !data.geminiConfigured) {
            statusText.textContent = 'API key not configured';
            (statusIndicator.querySelector('.status-dot') as HTMLElement).style.background = 'var(--warning)';
            showAlert('Please configure your GEMINI_API_KEY in the .env file', 'warning');
        }
    } catch {
        statusText.textContent = 'Server offline';
        (statusIndicator.querySelector('.status-dot') as HTMLElement).style.background = 'var(--error)';
